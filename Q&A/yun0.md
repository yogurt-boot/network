...
<details>
   <summary> Answer </summary>
<br />
   XSS는 사용자 입력값을 위변조하여 의도되지 않은 스크립트가 실행되도록 하는 공격이다.
   CSRF는 사용자가 인증된 상태를 이용하여 권한이 있는 사용자가 요청한 것처럼 위장해 의도되지 않은 요청을 보내는 공격이다.
</details>
...
<details>
   <summary> Answer </summary>
<br />
   해싱은 같은 메시지를 같은 알고리즘으로 연산하면 항상 같은 값이 반환된다는 특징을 이용해 두 값이 서로 같은 값인지 확인할 수 있도록 하는 기법이다. 
   대부분의 해싱 알고리즘은 복호화가 불가능하다. 많이 사용하는 알고리즘으로 MD, SHA 등이 있다.
   암호화는 특정 키나 알고리즘을 알지 못하는 외부인들이 메시지를 읽을 수 없도록 숨기는 기법이다. 복호화에 필요한 키를 알고있는 상대방만이 암호화된 메시지를 해독할 수 있다.
   암호화와 복호화에 사용되는 키에 따라 대칭 암호화, 비대칭 암호화로 분류할 수 있다. 많이 사용되는 암호화 기법으로는 AES 등이 있다.
</details>
